...
for (const m of gate.calls) {
  ...
  // العاقبة حسب المصدر (الدفعة الصفرية): المستودع يحذف ⇒ غائب⇒صفر · الفرع لا يحذف ⇒ «لا حذف / تبقى»
  if (/ملف المستودع/.test(m)) check(/غائب/.test(m) && /صفر/.test(m), "① حوار المستودع يشرح العاقبة (غائب ⇒ صفر)");
  else check(/لا حذف/.test(m) && /تبقى/.test(m), "① حوار الفرع يوضّح: لا حذف (الأكواد الناقصة تبقى)");
}
